...
describe('eBay used-market research', () => {
  it('covers every bandwidth-ranked GPU without inventing prices for missing matches', () => {
    ...
    expect(ebayUsedMarketSeeds.filter((snapshot) => snapshot.listings.length > 0)).toHaveLength(44);
    expect(ebayUsedMarketSeeds.flatMap((snapshot) => snapshot.listings)).toHaveLength(88);
  });
